...
class JacobianTransformation(ThreeDScene):
    def construct(self):
        ...
        surf_eq = MathTex(r"\mathbf{r}(u, v) = \langle u, v, 4 - u^2 - v^2 \rangle", font_size=8)
        partials = MathTex(r"\mathbf{r}_u = \langle 1, 0, -2u \rangle, \quad \mathbf{r}_v = \langle 0, 1, -2v \rangle", font_size=8)
        cross_prod = MathTex(r"\mathbf{r}_u \times \mathbf{r}_v = \langle 2u, 2v, 1 \rangle", font_size=8)

        # Arrange them in a column
        formula_stack = VGroup(surf_eq, partials, cross_prod, jac_label).arrange(DOWN, aligned_edge=LEFT, buff=0.1)
        ...
        # For r(u,v) = <u, v, 4 - u^2 - v^2>, the magnitude is sqrt(4u^2 + 4v^2 + 1)
        jac_num_value = always_redraw(lambda: DecimalNumber(
            np.sqrt(4 * u_tracker.get_value()**2 + 4 * v_tracker.get_value()**2 + 1),
